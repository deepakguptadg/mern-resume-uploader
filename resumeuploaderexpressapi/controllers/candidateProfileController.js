  ...
  static saveProfile = async (req, res) => {
    try {
      ...
      if (name && email && pimage && rdoc) {
        ...
        const resumeList = await CandidateProfileModel.find()
        res.status(201).send({ "status": "success", "message": "Profile Uploaded Successfully", "data": resumeList, "candidate": candidate })
      } else {
